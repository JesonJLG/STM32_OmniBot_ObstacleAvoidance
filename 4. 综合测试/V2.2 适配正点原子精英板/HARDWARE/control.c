...
extern u8 mode_flag;
extern char Lr_RxPacket[10];			//左摇杆数据接收缓冲区
extern char Rr_RxPacket[10];			//右摇杆数据接收缓冲区
extern char Pitch_Roll_RxPacket[20];	//APP偏航角数据接收缓冲区
...
//    printf("\r\n");
    ...
    if (Pwm_RxPacket[0] == 'P')
    {   //"P999*"
        pwm_limit = (Pwm_RxPacket[1]-'0')*100 + (Pwm_RxPacket[2]-'0')*10 + (Pwm_RxPacket[3]-'0'); 	//注意更新pwm_limit
        printf("pwm_limit:%d\t", pwm_limit);
        Serial_Printf("*G%d*", pwm_limit);
    }
    if (Lr_RxPacket[0] == 'L')
    {   //"LX50Y50*"
        Joy_Lx = (Lr_RxPacket[2] - '0') * 10 + (Lr_RxPacket[3] - '0');	//ACSII转int
...
{
    static char tmpflag = 0;	//太近了 测距盲区
    static float disMiddle, disLeft, disRight;
    sgReset();
