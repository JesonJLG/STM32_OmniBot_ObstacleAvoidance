...
extern u8 mode_flag;
extern char Lr_Buf[10];			//左摇杆数据接收缓冲区
extern char Rr_Buf[10];			//右摇杆数据接收缓冲区
...
//	printf("\r\n");
    int pwm_limit = (Pwm_RxPacket[2]-'0')*100 + (Pwm_RxPacket[3]-'0')*10 + (Pwm_RxPacket[4]-'0'); 	//注意更新pwm_limit
	printf("pwm_limit:%d\t", pwm_limit);
	Serial_Printf("*G%d*", pwm_limit); 
    if (Lr_Buf[0] == 'L')
    {   //"LX50Y50"
        Joy_Lx = (Lr_Buf[2] - '0') * 10 + (Lr_Buf[3] - '0');	//ACSII转int
        Joy_Ly = (Lr_Buf[5] - '0') * 10 + (Lr_Buf[6] - '0');
    }
    if (Rr_Buf[0] == 'R')
    {   //"RX50Y50"
        Joy_Rx = (Rr_Buf[2] - '0') * 10 + (Rr_Buf[3] - '0');
        Joy_Ry = (Rr_Buf[5] - '0') * 10 + (Rr_Buf[6] - '0');
    }
...
//    printf("\r\n");
    ...
    */
    ...
    /*------------限制正向最大占空比-------------*/
    if (pwm1 > 1000)	pwm1 = pwm_limit;
    if (pwm2 > 1000)	pwm2 = pwm_limit;
    if (pwm3 > 1000)	pwm3 = pwm_limit;
    if (pwm4 > 1000)	pwm4 = pwm_limit;
