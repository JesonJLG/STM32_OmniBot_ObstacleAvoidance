...
{
    Motor_PWM_Init(500,72);	//4个电机各占一个PWM引脚
    Motor_GPIO_Init();		//4个电机另外一个引脚推挽输出
    STBY_Init();			//2个待机引脚 推挽输出
}
...
{
    TIM_SetCompare1(TIM2,500-speed);	//TIM2_CH1->PA0		R_AIN1_PWM（speed越大，CCR越小，占空比越大，因为PWM2模式）
    R_AIN2_ON;							//R_AIN2:右下

    TIM_SetCompare2(TIM2,speed);		//TIM2_CH2->PA1
    R_BIN2_OFF;							//R_BIN2:左下

    TIM_SetCompare3(TIM2,speed);		//TIM2_CH3->PA2
    L_AIN2_OFF;							//L_AIN2:右上
