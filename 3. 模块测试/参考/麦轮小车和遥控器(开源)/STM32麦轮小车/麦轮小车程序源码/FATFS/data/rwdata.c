...
/**************************************************
函数名称：FATFS_ScanFilesType(u8 *fname)
函数功能：报告文件的类型
入口参数：fname:文件名
返回参数：0XFF,表示无法识别的文件类型编号.
...
void FATFS_RW_Text(void)
{
	...
	f_mkdir("0:\\Data");            //创建 (Data) 文件夹
	
	res=f_open(file,"0:\\Data\\data.txt",FA_CREATE_ALWAYS|FA_WRITE);//打开文件
	if(res!=0)printf("Open the failure ! , res=%d\n",res);
	
	res=f_write(file,"abcdefg",100,&bw);//向文件写入数据
	if(res!=0)printf("Write the failure ! , res=%d\n",res);
	f_close(file);                      //关闭文件
	
	
	res=f_open(file,"0:\\Data\\data.txt",FA_READ);//打开文件，只读
	if(res!=0)printf("Open the failure ! , res=%d\n",res);
